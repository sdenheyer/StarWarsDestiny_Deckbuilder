package com.example.starwarsdestinydeckbuilder.domain
...
class GetCardWithFormat @Inject constructor(val cardRepo: CardRepositoryImpl) {
    operator fun invoke(code: String): Flow<Resource<Card?>> = combineTransform(
        cardRepo.getCardFormats(),
        cardRepo.getCardbyCode(code)
            ...
            val reprints = card.reprints.map {
                when (it) {
                    is CodeOrCard.CodeValue -> {
                        val reprintcard = cardRepo.getCardbyCode(it.value).first { it.status == Resource.Status.SUCCESS }
            ...
            val parellelDiceOf = card.parallelDiceOf.map {
                when (it) {
                    is CodeOrCard.CodeValue -> {
                        ...
                        val dicecard = cardRepo.getCardbyCode(it.value).first { it.status == Resource.Status.SUCCESS }
